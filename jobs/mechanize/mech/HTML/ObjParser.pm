...
HTML::ObjParser - Alternative HTML::Parser interface

=head1 SYNOPSIS

require HTML::ObjParser;
$p = HTML::ObjParser->new("index.html") ||
die "Can't open: $!";
...
The C<HTML::ObjParser> is an alternative interface to the
...
=item $p = HTML::ObjParser->new( $filename, %opt );

=item $p = HTML::ObjParser->new( $filehandle, %opt );

=item $p = HTML::ObjParser->new( \$document, %opt );
...
C<HTML::ObjParser> can read() from when it needs more data.  Typically
it will be a filehandle of some kind.  The stream will be read() until
EOF, but not closed.

A newly constructed C<HTML::ObjParser> differ from its base classes
...
my $p = HTML::ObjParser->new( \$document );
# ...
...
between the <A>...</A> tags:
  ...
  use HTML::ObjParser;
  $p = HTML::ObjParser->new(shift||"index.html");
